...
public class PR131Main {
    ...
    private static Document construirDocument() {
        try {
            // Crear una nova instància de DocumentBuilderFactory
            ...
            // Crear l'element "llibre" amb l'atribut id
            Element llibre = doc.createElement("llibre");
            llibre.setAttribute("id", "001");
            rootElement.appendChild(llibre);

            // Afegir els elements de detall del llibre
    ...
    private static void guardarDocument(Document doc, File fitxerSortida) {
        try {
            // Crear una instància de TransformerFactory
            TransformerFactory transformerFactory = TransformerFactory.newInstance();
            Transformer transformer = transformerFactory.newTransformer();

            // Configurar la sortida per tenir espais d'indentació
            transformer.setOutputProperty(OutputKeys.INDENT, "yes");
            transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "4");

            // Crear la font i el resultat del fitxer
            ...
            System.out.println("El fitxer XML s'ha guardat correctament a " + fitxerSortida.getAbsolutePath());
